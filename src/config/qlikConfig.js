...
const qApp = async (config) => {
  try {
    ...
    return new Promise((resolve) => {
      if (globals.qlik) {
        const app = globals.qlik.openApp(config.appId, { ...config, isSecure: config.isSecure, prefix });
        resolve(app);
      } else {
        window.require(['js/qlik'], (q) => {
          ...
          const app = q.openApp(config.appId, { ...config, isSecure: config.isSecure, prefix });
          return resolve(app);
        });
      }
    });
...
const cApp = qApp(config);
const eApp = qDoc(config);
const QlikContext = React.createContext({
  cApp,
  eApp
});

export { config, sessionAppConfig, TurnkeyLogInAppID, TurnkeyCalcID, QlikContext };
